...
var (
	...
	//  Background
	On_Black  = Color("\033[40m%s\033[0m") //  Black
	On_Red    = Color("\033[41m%s\033[0m") //  Red
	On_Green  = Color("\033[42m%s\033[0m") //  Green
	On_Yellow = Color("\033[43m%s\033[0m") //  Yellow
	On_Blue   = Color("\033[44m%s\033[0m") //  Blue
	On_Purple = Color("\033[45m%s\033[0m") //  Purple
	On_Cyan   = Color("\033[46m%s\033[0m") //  Cyan
	On_White  = Color("\033[47m%s\033[0m") //  White
	...
	On_IBlack  = Color("\033[0;100m%s\033[0m") //  Black
	On_IRed    = Color("\033[0;101m%s\033[0m") //  Red
	On_IGreen  = Color("\033[0;102m%s\033[0m") //  Green
	On_IYellow = Color("\033[0;103m%s\033[0m") //  Yellow
	On_IBlue   = Color("\033[0;104m%s\033[0m") //  Blue
	On_IPurple = Color("\033[10;95m%s\033[0m") //  Purple
	On_ICyan   = Color("\033[0;106m%s\033[0m") //  Cyan
	On_IWhite  = Color("\033[0;107m%s\033[0m") //  White

)

func parseData(m interface{}) []byte {
	logs, _ := json.MarshalIndent(m, "", "")
	return logs
}

func Color(colorString string) func(m interface{}) {
	sprint := func(args interface{}) {
		logs := parseData(args)
		fmt.Println(fmt.Sprintf(colorString,
			fmt.Sprintf(time.Now().Format("02-01-2006 15:04:05")+" %s", logs)))
	}
	return sprint
}
